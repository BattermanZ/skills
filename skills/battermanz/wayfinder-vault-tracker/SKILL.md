...
- **Blocking edges are mirrored on both tickets.** A `Blocks:` entry on one side needs the matching `Blocked by:` on the other, or the board and the graph go wrong in opposite directions. Reframings that arrive as an upstream ticket resolves go under the question's `### Reframings` heading, dated and newest first, rather than rewriting the question in place.
